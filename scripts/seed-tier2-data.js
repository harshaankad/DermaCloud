 ...
 * Run with: node scripts/seed-tier2-data.js
 *
 * This script creates:
...
const ClinicSchema = new mongoose.Schema(
  {
    ...
    logo: String,
    seal: String,
    dermatologyFieldSettings: { type: mongoose.Schema.Types.Mixed, default: {} },
    dermatologyCustomFields: { type: Array, default: [] },
    cosmetologyFieldSettings: { type: mongoose.Schema.Types.Mixed, default: {} },
    cosmetologyCustomFields: { type: Array, default: [] },
  },
  { timestamps: true }
);
...
const ConsultationTemplateSchema = new mongoose.Schema(
  {
    ...
    templateType: { type: String, enum: ["dermatology", "cosmetology"], default: "dermatology", required: true },
...
const AIResultSchema = new mongoose.Schema(
  {
    predictions: [{ condition: String, probability: Number }],
...
const ConsultationDermatologySchema = new mongoose.Schema(
  {
    clinicId: { type: mongoose.Schema.Types.ObjectId, ref: "Clinic", required: true },
    ...
    followUp: { date: Date, reason: String },
    patientSummary: { aiGenerated: String, doctorEdited: String },
    consent: { obtained: Boolean, notes: String },
    doctorSignature: String,
    generatedFiles: { pdfUrl: String, wordUrl: String, generatedAt: Date },
...
const ConsultationTemplate = mongoose.models.ConsultationTemplate || mongoose.model("ConsultationTemplate", ConsultationTemplateSchema);
const ConsultationDermatology = mongoose.models.ConsultationDermatology || mongoose.model("ConsultationDermatology", ConsultationDermatologySchema);
...
async function seedDatabase() {
  try {
    ...
    const patients = await Promise.all(
      patientsData.map((p) => Patient.create({ ...p, clinicId: clinic._id }))
    );
    ...
    const consultationsData = [
      ...
      // Patient 5 (Vikram Singh) - Seborrheic Dermatitis
      {
        patientIndex: 4,
    ...
    console.log(`  - Tinea Corporis (Fungal infection) - with follow-up visit`);
