...
export default function FrontdeskLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/auth/frontdesk/login", {
        method: "POST",
        headers: {
      ...
      if (data.success) {
        localStorage.setItem("frontdeskToken", data.data.token);
        if (data.data.refreshToken) localStorage.setItem("frontdeskRefreshToken", data.data.refreshToken);
        localStorage.setItem("frontdeskStaff", JSON.stringify(data.data.staff));
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      const res = await fetch("/api/auth/google?role=frontdesk");
        ...
        <div className="bg-white rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100 border-l-4 border-l-teal-500 p-8">
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-1">
              <h2 className="text-2xl font-bold text-gray-900">Frontdesk Login</h2>
              <span className="px-2.5 py-0.5 text-xs font-semibold bg-teal-100 text-teal-700 rounded-full">Staff</span>
            </div>
            <p className="text-gray-500">Sign in to the clinic management portal</p>
          </div>
          ...
          <button
            ...
            type="button"
            className="w-full flex items-center justify-center gap-3 py-3 px-4 bg-white border border-gray-300 rounded-xl text-gray-700 font-medium hover:bg-gray-50 hover:border-gray-400 transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {googleLoading ? (
