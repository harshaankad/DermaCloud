...
export default function Tier2Dashboard() {
  ...
  const fetchDashboard = useCallback(async (showLoader = false, liteMode = false) => {
    ...
    try {
      const url = liteMode ? "/api/tier2/dashboard?lite=true" : "/api/tier2/dashboard";
  ...
  // Auto-polling every 30 seconds — lite mode only refreshes live data
  useEffect(() => {
    pollRef.current = setInterval(() => fetchDashboard(false, true), POLL_INTERVAL);
    return () => {
      ...
      <nav className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex gap-1 overflow-x-auto scrollbar-hide">
            {[
              { label: "Dashboard", href: "/clinic/dashboard", active: true },
          ...
          <div className="lg:col-span-1 order-2 lg:order-1">
            <div className="bg-white rounded-xl shadow-lg p-6">
              ...
              <div className="space-y-3">
                ...
                <Link
                  href="/clinic/consultations?filter=today"
                            ...
                            <div className="inline-flex flex-col items-center px-2.5 py-1.5 bg-gradient-to-b from-teal-50 to-cyan-50 rounded-xl border border-teal-100">
                              {(() => {
                                if (!apt.appointmentTime) {
                                  return <span className="text-sm font-bold text-teal-700 leading-tight">—</span>;
                                }
                                const [hStr, mStr] = apt.appointmentTime.split(":");
