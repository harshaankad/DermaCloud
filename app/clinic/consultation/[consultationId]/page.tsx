  ...
  const fetchConsultation = async () => {
    try {
      ...
      if (data.success) {
        ...
        try {
          const formRes = await fetch(`/api/tier2/settings/forms?formType=dermatology`, {
            headers: { Authorization: `Bearer ${token}` },
          });
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3.5">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3 min-w-0">
              <Link href="/clinic/consultations" className="p-2 hover:bg-gray-100 rounded-lg text-gray-400 hover:text-teal-600 transition-colors flex-shrink-0">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-5">
                  ...
                  <div key={idx} className={`rounded-2xl border-2 ${color.light} shadow-sm overflow-hidden`}>
                    ...
                    {isExpanded && (
                      <div className="bg-white p-5 space-y-5">
                        ...
                        {/* Prescription — standalone full-width table */}
                        {Array.isArray(fd.prescription) && fd.prescription.length > 0 && fd.prescription.some((m: any) => m.name?.trim()) && (
                          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                            <div className="px-5 py-3 border-b border-gray-100 flex items-center gap-2">
                              <div className={`w-1 h-4 rounded-full bg-gradient-to-b ${color.bar}`} />
                              <h4 className="text-sm font-bold text-gray-800">Prescription</h4>
                              <span className="ml-auto text-[11px] font-semibold text-teal-600 bg-teal-50 px-2 py-0.5 rounded-full">{fd.prescription.filter((m: any) => m.name?.trim()).length} medicine{fd.prescription.filter((m: any) => m.name?.trim()).length > 1 ? "s" : ""}</span>
                            </div>
                            <div className="overflow-x-auto">
                              <table className="w-full">
                                <thead>
                                  <tr className="bg-gray-50 border-b border-gray-100">
                                    <th className="px-5 py-3 text-left text-[11px] font-bold text-gray-400 uppercase tracking-wider w-10">#</th>
                                    <th className="px-5 py-3 text-left text-[11px] font-bold text-gray-400 uppercase tracking-wider">Medicine</th>
                                    <th className="px-5 py-3 text-left text-[11px] font-bold text-gray-400 uppercase tracking-wider">Dosage</th>
                                ...
                                <tbody className="divide-y divide-gray-50">
                                  {fd.prescription.filter((m: any) => m.name?.trim()).map((med: any, mi: number) => (
                                    <tr key={mi} className="hover:bg-teal-50/30 transition-colors">
                                      <td className="px-5 py-3.5 text-sm text-teal-600 font-bold">{mi + 1}</td>
                                      <td className="px-5 py-3.5 text-sm font-semibold text-gray-900">{med.name}</td>
                                      <td className="px-5 py-3.5 text-sm text-gray-600">{med.dosage || "—"}</td>
                          ...
                          <div>
                            ...
                            <div className="space-y-4">
                              ...
                              {(() => {
                                ...
                                const extras = Object.entries(fd).filter(([key, val]) => !allConfigFields.includes(key) && !["prescription"].includes(key) && val && (typeof val === "string" ? val.trim() : typeof val !== "object"));
                                    ...
                                    <div className="space-y-2">
                                      {extras.map(([key, val]) => (
                                        <div key={key} className="bg-gray-50 rounded-lg px-4 py-2.5">
                                          <p className="text-xs text-gray-400 mb-0.5">{key.replace(/([A-Z])/g, " $1").replace(/^./, (s) => s.toUpperCase())}</p>
        ...
        {!isMultiIssue && (
          <>
            ...
            {/* Prescription — full-width standalone section */}
            {(() => {
              const rxData = consultation.customFields?._issues?.[0]?.formData?.prescription || consultation.customFields?.prescription;
              const rxMeds = Array.isArray(rxData) ? rxData.filter((m: any) => m.name?.trim()) : [];
              if (rxMeds.length === 0) return null;
              return (
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
                    <div className="w-1.5 h-5 rounded-full bg-gradient-to-b from-teal-500 to-emerald-500"></div>
                    <h2 className="text-sm font-bold text-gray-800">Prescription</h2>
                    <span className="ml-auto text-xs font-semibold text-teal-600 bg-teal-50 px-2.5 py-0.5 rounded-full">{rxMeds.length} medicine{rxMeds.length > 1 ? "s" : ""}</span>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      <thead>
            ...
            {/* All Details — every field grouped by form section (single-issue) */}
            {formSections.length > 0 && consultation.customFields && (() => {
              const cf = consultation.customFields;
              // For single-issue, form data may be in _issues[0].formData or flat in customFields
