/**
 * Generates a PDF report for a dermatology consultation, uploads it to S3,
 * and returns a 7-day signed URL suitable for sharing via WhatsApp.
 */
...
function textBlock(doc: PDFKit.PDFDocument, text: string, titleColor = C.navy, useIndicFont = false) {
  ...
  const renderLineWithBold = (line: string, x: number, fontSize: number, color: string, maxWidth: number) => {
    ...
    if (totalWidth <= maxWidth) {
      ...
      doc.fillColor(color).font(bodyFont).fontSize(fontSize)
         .text(plainText, x, doc.y, { width: maxWidth, lineGap: useIndicFont ? 4 : 2 });
    }
  };
...
export async function POST(request: NextRequest) {
  try {
    ...
    const fileName  = `Consultation_${patientId}_${dateStr}_${Date.now()}.pdf`;
